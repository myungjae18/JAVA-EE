...
<%
	...
	List<Product> productList=productDAO.selectAllByFkey(subcategory_id);
%>
<!DOCTYPE html>
<html lang="en">
<head>
							...
							<div class="products_bar_content d-flex flex-column flex-xxl-row align-items-start align-items-xxl-center justify-content-start">
								<div class="product_categories">
									<ul class="d-flex flex-row align-items-start justify-content-start flex-wrap">
										...
										%>
										<%
											List<Subcategory> subList=subDAO.selectAllByFk(Integer.parseInt(topcategory_id));
										%>
										<%for(int i=0;i<subList.size();i++){%>
										<%Subcategory subcategory=subList.get(i); %>
										<li ><a href="/category.jsp?topcategory_id=<%=topcategory_id%>&subcategory_id=<%=subcategory.getSubcategory_id()%>"><%=subcategory.getSub_name() %></a></li>
										<%}%>										
									</ul>
								</div>
